...
class PongGame:
    ...
    def reset(self):
        ...
        self._update_grid()
        return self.racket_l.pos[0], *self.ball.pos, *self.ball.velocity
    ...
    def step(self, action, wind=True, left_bot=False, right_bot=False):
        ...
        self._update_grid()
        self.env_state = self.racket_l.pos[0], *self.ball.pos, *self.ball.velocity
    ...
    def _move_ball(self, wind):
        ...
        if wind and random.random() < 0.05 and 5 < self.ball.pos[1] < SCREEN_WID - 5:
    ...
    def _take_action(self, action, racket="Left"):
        ...
        if action == 0:
            racket.pos[0] -= racket.velocity[0]
            if racket.pos[0] < racket.len:
                racket.pos[0] = racket.len
        elif action == 1:
            racket.pos[0] += racket.velocity[0]
            if racket.pos[0] > SCREEN_HEI - racket.len:
                racket.pos[0] = SCREEN_HEI - racket.len
        elif action == 2:
            pass
        else:
            raise Exception
    ...
    def _is_collision(self, agent="Left"):
        res = None
        if self.racket_l.pos[0] - self.racket_l.len <= self.ball.pos[0] < self.racket_l.pos[0] + self.racket_l.len:
            if self.racket_l.pos[1] + 1 == self.ball.pos[1]:
                res = agent == "Left"
        if self.racket_r.pos[0] - self.racket_r.len <= self.ball.pos[0] < self.racket_r.pos[0] + self.racket_r.len:
            if self.racket_r.pos[1] - 1 == self.ball.pos[1]:
                res = agent == "Right"
